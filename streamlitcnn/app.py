...
if cnn_model is None:
    st.sidebar.warning("⚠️ Model CNN `cnn_signal_model.h5` belum dibuat. Silakan jalankan `train_cnn.py` untuk melatih model!")
else:
    st.sidebar.success("🤖 Model CNN `cnn_signal_model.h5` Berhasil Dimuat!")
...
if page == "Analisis Sinyal":
    ...
    if mode == "Unggah Gambar Spektrum":
                ...
                try:
                    ...
                    with col_metrics:
                        ...
                        if res['signals']:
                            ...
                            # Predict signal type (AM vs FM) using CNN model
                            sig_type, confidence = cnn.predict_signal_type(img_bgr, panel)
                            ...
                            <div style="display: flex; gap: 1rem; margin-bottom: 1.5rem;">
                                <div>
                                    ...
                                    <span class="badge {badge_type}">{sig_type} (CNN: {confidence:.1f}%)</span>
                                </div>
                                <div>
        ...
        with col_results:
            ...
            # Predict AM/FM using rule-based bandwidth fallback (no image for CNN)
            sig_type = "FM" if bandwidth_val > 35 else "AM"
            ...
            <div style="display: flex; gap: 1rem; margin-bottom: 1.5rem;">
                <div>
                    <span style="font-size:0.9rem; color:#8892b0; display:block; margin-bottom:0.2rem;">Tipe Sinyal</span>
                    <span class="badge {badge_type}">{sig_type} (Heuristic)</span>
                </div>
                <div>
                    <span style="font-size:0.9rem; color:#8892b0; display:block; margin-bottom:0.2rem;">Kualitas TIPHON</span>
